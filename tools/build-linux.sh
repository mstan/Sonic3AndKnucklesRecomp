...
APP_NAME="Sonic3K"
CMAKE_TARGET="Sonic3KRecomp"
ROM_EXTS="md bin gen"                       # AppRun auto-finds the Genesis ROM next to the AppImage
EXTRA_ARGS=""
REGEN_CMD=""                               # generated C is committed; regen is the Windows recompiler+disasm
PREBUILD_CMD=""
POSTBUILD_CMD=""
# prod/debug -> framework cmake flags. Release strips ALL tooling: GEN_ENABLE_TRACE
# OFF (cmd_server TCP + frame_snapshots), GEN_DEV_TRACE OFF (chip_trace self-stubs),
# SONIC_REVERSE_DEBUG OFF. Only the NATIVE target is built (AGPL-free, no oracle).
PROD_CMAKE_FLAGS=( -DGEN_ENABLE_TRACE=OFF -DGEN_DEV_TRACE=OFF -DSONIC_REVERSE_DEBUG=OFF )
DEBUG_CMAKE_FLAGS=( -DGEN_ENABLE_TRACE=ON -DGEN_DEV_TRACE=ON -DSONIC_REVERSE_DEBUG=OFF )
# ============================================================================
